...
(InputArray image, InputArray templ,
    ...
    float init_x, float init_y, float init_rot,
    float std_x,  float std_y,  float std_rot,
    vector<float> &  dispAndRot,
    int motionType,
    cv::TermCriteria criteria
)
{
    ...
    std::normal_distribution<float> d1(init_x, std_x);
    std::normal_distribution<float> d2(init_y, std_y);
    std::normal_distribution<float> d3(init_rot, std_rot);
    ...
    while (true)
    {
        iTrial += 1;
        if (iTrial > 1) {
            printf("# ECC fails at guess (%f, %f, %f).", dx, dy, rz * 180.f / 3.14159265359f);
            dx = d1(g1);
            dy = d2(g2);
            rz = d3(g3) * (3.14159265359f / 180.f);
            printf("# Next ECC trialis (%f, %f, %f).", dx, dy, rz * 180.f / 3.14159265359f);
        }
        tmp1 = (Mat_<float>(3, 3) << 1, 0, -rx, 0, 1, -ry, 0, 0, 1);
        tmp2 = (Mat_<float>(3, 3) << cos(rz), sin(rz), 0, -sin(rz), cos(rz), 0, 0, 0, 1);
        tmp3 = (Mat_<float>(3, 3) << 1, 0, dx, 0, 1, dy, 0, 0, 1);
        initWarp33 = tmp3 * tmp2 * tmp1;
        ...
        break;
    } // while loop of ECC calls

    // If the program runs here, it means ECC succeeds.
    if (warpMat.rows == 3)
        warp33 = warpMat;
    else
        warp33 = (Mat_<float>(3, 3) <<
